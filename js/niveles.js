...
const MUNDOS = [
    ...
    // =========================================================
    { id: 'mprac', titulo: 'Práctica: de lo simple a los JOINs a fondo', insignia: '🧮',
      descripcion: 'Un repaso variado: consultas simples, combinaciones más elaboradas de varias tablas, y un recorrido por cada tipo de JOIN comparado con el producto cartesiano.',
      niveles: [
        {
            id: 'mpracn1', titulo: 'Los mejores salarios',
            mision: 'Recursos Humanos quiere revisar los sueldos más altos: muestra todos los empleados con salario mayor a 2000, ordenados del más alto al más bajo.',
            concepto: 'Consulta simple: WHERE + ORDER BY',
            sintaxis: 'SELECT * FROM tabla WHERE condicion ORDER BY columna DESC;',
            tablas: ['empleados'], tipo: 'consulta', ordenImporta: true,
            solucion: 'SELECT * FROM empleados WHERE salario > 2000 ORDER BY salario DESC;',
            pistas: [
                'Filtra con WHERE salario > 2000 y luego ordena con ORDER BY salario DESC.',
                'SELECT * FROM empleados WHERE salario > 2000 ORDER BY salario DESC;',
            ],
        },
        {
            id: 'mpracn2', titulo: 'Pedidos de este año',
            mision: "Contabilidad quiere revisar solo los pedidos realizados durante 2024. Encuentra los pedidos cuya fecha empiece con '2024'.",
            concepto: 'Consulta simple: LIKE sobre una fecha en texto',
            sintaxis: "SELECT * FROM tabla WHERE columna LIKE 'patron%';",
            tablas: ['pedidos'], tipo: 'consulta',
            solucion: "SELECT * FROM pedidos WHERE fecha LIKE '2024%';",
            pistas: [
                'La fecha se guarda como texto en formato \'YYYY-MM-DD\', así que puedes buscar el patrón con LIKE.',
                "SELECT * FROM pedidos WHERE fecha LIKE '2024%';",
            ],
        },
        {
            id: 'mpracn3', titulo: 'Categorías en orden',
            mision: 'Para un menú desplegable en la web de la tienda, necesitas la lista de categorías ordenada alfabéticamente por nombre.',
            concepto: 'Consulta simple: ORDER BY alfabético',
            sintaxis: 'SELECT * FROM tabla ORDER BY columna ASC;',
            tablas: ['categorias'], tipo: 'consulta', ordenImporta: true,
            solucion: 'SELECT * FROM categorias ORDER BY nombre ASC;',
            pistas: [
                'ORDER BY columna ASC ordena de la A a la Z (y es el orden por defecto, así que ASC es opcional).',
                'SELECT * FROM categorias ORDER BY nombre ASC;',
            ],
        },
        {
            id: 'mpracn4', titulo: 'Los proveedores más grandes',
            mision: 'Almacén quiere identificar a sus proveedores más importantes por volumen: muestra el nombre de cada proveedor junto con la SUMA del stock de todos sus productos, pero solo para los proveedores cuyo stock total supere las 100 unidades, ordenado de mayor a menor.',
            concepto: 'Consulta compleja con 2 tablas: JOIN + GROUP BY + HAVING + ORDER BY',
            sintaxis: 'SELECT b.nombre, SUM(a.columna) AS total FROM a JOIN b ON ... GROUP BY b.id HAVING SUM(a.columna) > n ORDER BY total DESC;',
            tablas: ['productos', 'proveedores'], tipo: 'consulta', ordenImporta: true, ignorarNombresColumnas: false,
            solucion: 'SELECT pr.nombre, SUM(p.stock) AS stock_total FROM productos p JOIN proveedores pr ON p.proveedor_id = pr.id GROUP BY pr.id HAVING SUM(p.stock) > 100 ORDER BY stock_total DESC;',
            pistas: [
                'Primero une productos con proveedores, luego agrupa por proveedor y suma el stock con SUM().',
                'Filtra los GRUPOS (no las filas) con HAVING SUM(p.stock) > 100, y dale a la columna calculada el alias exacto stock_total.',
                'SELECT pr.nombre, SUM(p.stock) AS stock_total FROM productos p JOIN proveedores pr ON p.proveedor_id = pr.id GROUP BY pr.id HAVING SUM(p.stock) > 100 ORDER BY stock_total DESC;',
            ],
        },
        {
            id: 'mpracn5', titulo: 'Ventas de Electrónica por ciudad',
            mision: "Gerencia quiere saber qué ciudades compran más productos de Electrónica: para pedidos en estado 'completado' de productos de la categoría Electrónica (categoria_id = 1), muestra la ciudad del cliente y el total vendido (cantidad × precio_unitario, sumado), ordenado de mayor a menor.",
            concepto: 'Consulta compleja con 4 tablas: JOIN múltiple + GROUP BY + agregación',
            sintaxis: 'SELECT c.ciudad, SUM(d.cantidad * d.precio_unitario) AS total FROM clientes c JOIN pedidos p ON ... JOIN detalle_pedidos d ON ... JOIN productos prod ON ... WHERE ... GROUP BY c.ciudad ORDER BY total DESC;',
            tablas: ['clientes', 'pedidos', 'detalle_pedidos', 'productos'], tipo: 'consulta', ordenImporta: true, ignorarNombresColumnas: false,
            solucion: "SELECT c.ciudad, SUM(d.cantidad * d.precio_unitario) AS total FROM clientes c JOIN pedidos p ON c.id = p.cliente_id JOIN detalle_pedidos d ON p.id = d.pedido_id JOIN productos prod ON d.producto_id = prod.id WHERE p.estado = 'completado' AND prod.categoria_id = 1 GROUP BY c.ciudad ORDER BY total DESC;",
            pistas: [
                'Encadena los cuatro JOIN: clientes → pedidos → detalle_pedidos → productos (necesitas productos para filtrar por categoria_id).',
                "Filtra con WHERE p.estado = 'completado' AND prod.categoria_id = 1, agrupa por ciudad, y dale a la columna calculada el alias exacto total.",
                "SELECT c.ciudad, SUM(d.cantidad * d.precio_unitario) AS total FROM clientes c JOIN pedidos p ON c.id = p.cliente_id JOIN detalle_pedidos d ON p.id = d.pedido_id JOIN productos prod ON d.producto_id = prod.id WHERE p.estado = 'completado' AND prod.categoria_id = 1 GROUP BY c.ciudad ORDER BY total DESC;",
            ],
        },
        {
            id: 'mpracn6', titulo: 'El producto cartesiano',
            mision: "Antes de seguir con más JOINs, vas a ver con tus propios ojos qué es el 'producto cartesiano': combina CADA producto con CADA categoría, sin ninguna condición que los relacione. El resultado tendrá muchísimas más filas que productos hay (24 productos × 6 categorías = 144 combinaciones), porque se genera CADA combinación posible entre las dos tablas.",
            concepto: 'Producto cartesiano con CROSS JOIN',
            sintaxis: 'SELECT ... FROM tablaA CROSS JOIN tablaB;  -- combina cada fila de A con cada fila de B, sin condicion alguna',
            tablas: ['productos', 'categorias'], tipo: 'consulta',
            solucion: 'SELECT productos.nombre, categorias.nombre FROM productos CROSS JOIN categorias;',
            pistas: [
                'CROSS JOIN no lleva ON: simplemente combina cada fila de una tabla con cada fila de la otra.',
                'Si productos tiene 24 filas y categorias tiene 6, el resultado tendrá 24 × 6 = 144 filas.',
                'SELECT productos.nombre, categorias.nombre FROM productos CROSS JOIN categorias;',
            ],
        },
        {
            id: 'mpracn7', titulo: 'El subconjunto que sí coincide',
            mision: 'Ahora agrega la condición real que relaciona cada producto con su categoría. Verás que INNER JOIN se queda solo con las combinaciones donde SÍ coincide productos.categoria_id con categorias.id: de las 144 combinaciones del producto cartesiano anterior, aquí solo sobreviven las 24 que son ciertas (exactamente una por producto).',
            concepto: 'INNER JOIN = producto cartesiano filtrado por ON',
            sintaxis: 'SELECT ... FROM tablaA JOIN tablaB ON tablaA.clave = tablaB.clave;',
            tablas: ['productos', 'categorias'], tipo: 'consulta',
            solucion: 'SELECT productos.nombre, categorias.nombre FROM productos JOIN categorias ON productos.categoria_id = categorias.id;',
            pistas: [
                'INNER JOIN (el JOIN normal) es, por dentro, un producto cartesiano al que luego se le aplica la condición ON como filtro.',
                'Compara: la misión anterior (CROSS JOIN) te dio 144 filas; esta, con la condición ON, te debería dar solo 24.',
                'SELECT productos.nombre, categorias.nombre FROM productos JOIN categorias ON productos.categoria_id = categorias.id;',
            ],
        },
        {
            id: 'mpracn8', titulo: 'Nadie se queda sin fila (LEFT JOIN)',
            mision: 'Del producto cartesiano de TODOS los empleados con TODOS los pedidos, LEFT JOIN se queda solo con las combinaciones donde empleados.id = pedidos.empleado_id, pero sin perder a los empleados que nunca gestionaron ningún pedido (esos aparecen con NULL en los datos del pedido). Muestra el nombre de cada empleado junto con el id de cada pedido que gestionó.',
            concepto: 'LEFT JOIN = conserva todo el lado izquierdo',
            sintaxis: 'SELECT ... FROM tablaA LEFT JOIN tablaB ON tablaA.clave = tablaB.clave;',
            tablas: ['empleados', 'pedidos'], tipo: 'consulta',
            solucion: 'SELECT empleados.nombre, pedidos.id FROM empleados LEFT JOIN pedidos ON empleados.id = pedidos.empleado_id;',
            pistas: [
                'LEFT JOIN conserva TODAS las filas de la tabla de la izquierda (empleados), tengan o no coincidencia en pedidos.',
                'Los gerentes y almacenistas nunca gestionan pedidos directamente, así que deberían aparecer con NULL en el id de pedido.',
                'SELECT empleados.nombre, pedidos.id FROM empleados LEFT JOIN pedidos ON empleados.id = pedidos.empleado_id;',
            ],
        },
        {
            id: 'mpracn9', titulo: 'La misma idea, del otro lado (RIGHT JOIN)',
            mision: 'RIGHT JOIN es la misma idea que LEFT JOIN, pero mirando la relación desde el otro lado: conserva TODAS las filas de la tabla de la DERECHA. Escribe una consulta con pedidos RIGHT JOIN clientes que muestre el id de cada pedido junto con el nombre del cliente, sin perder a los clientes que no tienen ningún pedido.',
            concepto: 'RIGHT JOIN = conserva todo el lado derecho',
            sintaxis: 'SELECT ... FROM tablaA RIGHT JOIN tablaB ON tablaA.clave = tablaB.clave;',
            tablas: ['pedidos', 'clientes'], tipo: 'consulta',
            solucion: 'SELECT pedidos.id, clientes.nombre FROM pedidos RIGHT JOIN clientes ON pedidos.cliente_id = clientes.id;',
            pistas: [
                'RIGHT JOIN conserva TODAS las filas de la tabla que escribes DESPUÉS de RIGHT JOIN (aquí, clientes).',
                'Fíjate que el resultado tiene el mismo número de filas que si hubieras escrito clientes LEFT JOIN pedidos: son dos formas de llegar al mismo resultado.',
                'SELECT pedidos.id, clientes.nombre FROM pedidos RIGHT JOIN clientes ON pedidos.cliente_id = clientes.id;',
            ],
        },
        {
            id: 'mpracn10', titulo: 'Nadie se queda afuera (FULL JOIN)',
            mision: 'FULL JOIN conserva TODAS las filas de AMBOS lados, tengan o no coincidencia. Usando una autounión de empleados (e = empleado, j = jefe), muestra el nombre del empleado junto con el nombre de su jefe. Con FULL JOIN aparecerán a la vez: empleados sin jefe (como la Gerente General, con jefe_id NULL) Y empleados que nunca son jefe de nadie (vendedores y almacenistas), cada uno con NULL en el lado que le falta.',
            concepto: 'FULL JOIN = conserva ambos lados',
            sintaxis: 'SELECT ... FROM tablaA a FULL JOIN tablaB b ON a.clave = b.clave;',
            tablas: ['empleados'], tipo: 'consulta',
            solucion: 'SELECT e.nombre, j.nombre FROM empleados e FULL JOIN empleados j ON e.jefe_id = j.id;',
            pistas: [
                'Es una autounión, igual que en el organigrama, pero con FULL JOIN en vez de JOIN normal.',
                'Compara con un LEFT JOIN de la misma consulta: FULL JOIN debería devolver más filas, porque además agrega a los empleados que nunca aparecen como jefe de nadie.',
                'SELECT e.nombre, j.nombre FROM empleados e FULL JOIN empleados j ON e.jefe_id = j.id;',
            ],
        },
      ],
    },
